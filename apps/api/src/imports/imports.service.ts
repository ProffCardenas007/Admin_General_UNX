...
export class ImportsService {
	...
	async getImportById(importId: string) {
		const found = await this.importsRepository.findOne({ where: { id: importId } });
		if (!found) {
	...
	private validateRow(row: ParsedRow) {
		...
		if (Number.isNaN(progress) || progress < 0 || progress > 100) {
			throw new BadRequestException('progress_percent must be between 0 and 100');
		}
	...
	private async parseFile(fileName: string, fileBuffer: Buffer): Promise<ParsedRow[]> {
